...
#### Step 3 — Verify span relationships via CLI

```bash
# wait ~5s after sending, then run:
...
for i, p in enumerate(producers):
    ...
    print(f'msg[{i}]  producer={p[\"spanID\"][:8]}  parent={parent[:8]}(root)  '
          f'consumer={c[\"spanID\"][:8] if c else \"?\"}  '
...
Trace ID:  8defd309...
Spans:     29
Root span: 98a4986c  (POST /api/orders/batch-test)

msg[0]  producer=3d15191c  parent=98a4986c(root)  consumer=02c29abd  action=batch.audit_log
msg[1]  producer=97443019  parent=98a4986c(root)  consumer=1ee1d461  action=batch.cache_warm
msg[2]  producer=88681199  parent=98a4986c(root)  consumer=319bc428  action=batch.audit_log
msg[3]  producer=da00161f  parent=98a4986c(root)  consumer=eccb28ba  action=batch.audit_log
msg[4]  producer=1430f2ba  parent=98a4986c(root)  consumer=88abc705  action=batch.restock_alert

All spans share same trace_id: True
All span_ids unique: True
```
